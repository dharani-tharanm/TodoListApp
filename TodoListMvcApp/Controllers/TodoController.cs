...
namespace TodoListMvcApp.Controllers
{
    public class TodoController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                tasks = await _httpClient.GetFromJsonAsync<List<Todo>>("http://127.0.0.1:5000/tasks");
            }
        ...
        public async Task<IActionResult> Create(Todo todo)
        {
            ...
            try
            {
                var response = await _httpClient.PostAsJsonAsync("http://127.0.0.1:5000/tasks", todo);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var response = await _httpClient.DeleteAsync($"http://127.0.0.1:5000/tasks/{id}");
                response.EnsureSuccessStatusCode();
            }
            catch (HttpRequestException e)
            {
